...
static int LuaTapKey(lua_State* L) {
  ...
  if (!lua_isnumber(L, -1)) {
    Log(LOG_ERROR, "tap_key param must be a valid keycode");
    lua_pop(L, 1);
    return 1;
  }
...
static void DeclareGlobals(void) {
  ...
  DECLARE_KEYCODE(L, KC_VOLUME_MUTE);
  DECLARE_KEYCODE(L, KC_VOLUME_DOWN);
  DECLARE_KEYCODE(L, KC_VOLUME_UP);
  DECLARE_KEYCODE(L, KC_MEDIA_NEXT_TRACK);
  DECLARE_KEYCODE(L, KC_MEDIA_PREV_TRACK);
  DECLARE_KEYCODE(L, KC_MEDIA_STOP);
  DECLARE_KEYCODE(L, KC_MEDIA_PLAY_PAUSE);
  DECLARE_KEYCODE(L, KC_LAUNCH_MAIL);

  DECLARE_KEYCODE(L, KC_BROWSER_BACK);
  DECLARE_KEYCODE(L, KC_BROWSER_FORWARD);
  DECLARE_KEYCODE(L, KC_BROWSER_REFRESH)
  DECLARE_KEYCODE(L, KC_BROWSER_STOP);
  DECLARE_KEYCODE(L, KC_BROWSER_SEARCH);
  DECLARE_KEYCODE(L, KC_BROWSER_FAVORITES);
  DECLARE_KEYCODE(L, KC_BROWSER_HOME);
